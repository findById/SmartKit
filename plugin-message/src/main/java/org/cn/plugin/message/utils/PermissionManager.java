...
public class PermissionManager {
    ...
    private static Activity mContext = null; //ApplicationContext

    public static void init(Activity ctx) {
        PermissionManager.mContext = ctx;
    }
    ...
    public static void requestPermissions(Activity activity, OnPermissionsCallback onPermissionsCallback, String... permissions) {
        ...
        if (checkSelfPermission(permissions).length == 0) {
            //都已经授权了
        ...
        if (entity != null) {
            ...
            for (int i = 0; i < permissions.length; i++) {
                ...
                if (grantResult != PackageManager.PERMISSION_GRANTED) { //没有授权
                    success = false;
                    if (checkActivity(mContext)) {
                        showRequestRationale = !ActivityCompat.shouldShowRequestPermissionRationale(mContext, permission);
                    }
                }
